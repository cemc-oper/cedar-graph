...
# 500 hPa 高度场 + 海平面气压（`cn.h_500_psl`）

本图由配方 `cedar_graph/recipes/cn/h_500_psl.yaml` 驱动——两场加载、
`smth9` 九点平滑（repeat 4/2）、`style_units` 单位换算
（dagpm/hPa），588 特征线由样式库 `h_500.yml` 的 highlight 展开。
