...
本图由配方 `cedar_graph/recipes/cn/t2m.yaml` 驱动——配方经
{func}`~cedarkit.plots.engine.loader.get_plot_definition` 加载后
暴露与 Python 绘图模块相同的三件套接口（`PlotMetadata` /
`load_data` / `plot`）。运行时按起报月份自动选择夏/冬色标。
